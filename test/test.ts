...
function parseJson(fileName: string){
    ...
    const rawdata = fs.readFileSync(path.join(__dirname, mockDir, fileName));
    const mockData= JSON.parse(rawdata as unknown as string);
    return mockData;
}
...
function testSuite(){
    ...
    describe('cancelOrder', function() {
        it('cancel an open pending order', (done) => {
            kc.cancelOrder(Varieties.TEST, 100)
            .then(function(response: AnyObject) {
                expect(response).to.have.property('order_id');
                return done();
            }).catch(done); 
    ...
    describe('convertPosition', function() {
        it('convert existing position', (done) => {
            kc.convertPosition ({
                ...
                'quantity':1,
                'old_product': Products.CNC,
                'new_product': Products.MIS,
            })
            .then(function(response: AnyObject) {
